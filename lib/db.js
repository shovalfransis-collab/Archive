...
// (links), imageUrl (photos), content (notes), note (a free-text annotation
// on top of any item type, e.g. "watch this before the meeting"). Any field
// not relevant to the item's type is simply left null.
export async function createItem(data) {
  ...
  const rows = await sql`
    INSERT INTO items
      (folder_id, type, title, url, favicon_url, description, image_url, content, note)
    VALUES
      (${folderId}, ${type}, ${title}, ${url}, ${faviconUrl}, ${description}, ${imageUrl}, ${content}, ${note})
    RETURNING *`;
  return rows[0];
}
...
// Updates the free-text annotation on any item (link, photo, or note) —
// distinct from `content`, which is the body of a note-type item itself.
export async function updateItemNote(id, note) {
  const rows = await sql`
    UPDATE items SET note = ${note}, updated_at = now()
    WHERE id = ${id}
    RETURNING *`;
  return rows[0];
}
...
export async function searchItems(query) {
  ...
  return sql`
    SELECT * FROM items
    WHERE title ILIKE ${pattern} OR content ILIKE ${pattern} OR url ILIKE ${pattern} OR note ILIKE ${pattern}
